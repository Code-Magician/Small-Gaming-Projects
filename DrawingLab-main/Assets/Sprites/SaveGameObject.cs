...
public class SaveGameObject : MonoBehaviour
{
    ...
    public IEnumerator SaveChildrenIenum()
    {
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (child.name.Contains("Water"))
                Dlrs.Softbodies.Add(saveSB(child, "Water"));
            ...
            if (child.name.Contains("Rope"))
                Dlrs.Savedropes.Add(saveRope(child));

            yield return null;
        }

        yield return null;
        //SerializationUtility.SerializeValue(lrs, DataFormat.JSON);
    ...
    public saveRope saveRope(Transform rope)
    {
        ...
        Rope rope_comp = rope.GetComponent<Rope>();

        saverope.position = rope.position;

        saverope.startposition = rope_comp.pointStart;
        saverope.endposition = rope_comp.pointEnd;

        saverope.color = rope_comp.color;
    ...
    public void loadropes(saveRope sr)
    {
        ...
        Rope rope_comp = rope.GetComponent<Rope>();
        rope_comp.generateRope(sr.startposition, sr.endposition, sr.color);
    }
...
{
    public Vector3 position, hookSposition, hookEposition;
