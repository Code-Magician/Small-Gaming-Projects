...
public class GameController : MonoBehaviour
{
    ...
    void createWalls()
    {
        Vector3 start = new Vector3(-3 * width, -3 * height, -1f);
        Vector3 finish = new Vector3(-3 * width, +3 * height, -1f);
        createWall(start, finish);

        start = new Vector3(-3 * width, -3 * height, -1f);
        finish = new Vector3(+3 * width, -3 * height, -1f);
        createWall(start, finish);
    ...
    void createEgg(bool isGolden = false)
    {
        for (int i = 1; i <= 10; i++)
        {
            float x = Random.Range(-3 * width + 0.6f, 3 * width - 0.6f);
            float y = Random.Range(-3 * height + 0.6f, 3 * height - 0.6f);
    ...
    void levelUp()
    {
        ...
        destroyAllSpikes();
        createSpikes();
        noOfEggsForNextLvl = 10 + (levelNo * 2);
